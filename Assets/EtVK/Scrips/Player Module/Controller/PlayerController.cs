...
namespace EtVK.Player_Module.Controller
{
    public class PlayerController : MonoBehaviour, IMoveComponent
    {
        ...
        public void UpdateNormalMovement(Vector2 movement, float speed = 0f, float rotationSpeed = 0f)
        {
            ...
            var targetAngle = Mathf.Atan2(movement.x, movement.y) * Mathf.Rad2Deg +
                              playerManager.CameraMainTransform.eulerAngles.y;
        ...
        public void Jump(float gravityValue = 0f, float gravityMultiplayer = 0f, float jumpHeight = 0f)
        {
            ...
            gravityMultiplayer = gravityMultiplayer == 0f
                ? playerManager.GetLocomotionData().GravityMultiplayer
                : gravityMultiplayer;
        ...
        public void MoveOnSlope()
        {
            ...
            {
                characterController.Move(Vector3.down * characterController.height / 2 *
                                         playerManager.GetLocomotionData().SlopeFore * Time.deltaTime);
            }
        ...
        private void SteepSlopeMovement()
        {
            ...
            Move((moveDirection + Vector3.down * characterController.height / 2 *
                playerManager.GetLocomotionData().SlopeFore) * Time.deltaTime);
        }
